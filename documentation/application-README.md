...
## UPDATED OVERVIEW
This style and usage still technically works and however including **at4dx** and **force di** actually streamlines this particular Layer. Application is now a class in the a **at4dx** package and utilizes custom metadata for each factory in the application to bind the respective Domain, Service, Selector, and Unit Of Work SObjects list to the Application.

Which means everytime a new Selector, Domain, Service, or SObject is created and needs to be bound to Application, it is simply done by creating a new Custom Metadata record and the Application class no longer needs to be contantly updated.

Everything below can be effectively ignored if using **at4dx**
